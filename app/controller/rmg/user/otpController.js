...
async function checkPreviouOtp(playerId) {
  try {
    ...
    let previousOtp = await pgConnect.executeQuery('loyalty',query);
...
async function createSms(appId, playerId, number, message) {
  try {
    ...
    let createdSms = await pgConnect.executeQuery('loyalty',query);
...
async function createOtp(playerId, smsId, otpPin, isNewUser) {
  try {
    ...
    let createdOtp = await pgConnect.executeQuery('loyalty',query);
...
async function updateSms(smsId, gatewayResponse) {
  try {
    ...
    let updateOtp = await pgConnect.executeQuery('loyalty',query);
...
async function otpRetryLog(otpId, otpPin) {
  try {
    ...
    let otpRetry = await pgConnect.executeQuery('loyalty',query);
...
async function verifyOtp(optId, optPin) {
  try {
    ...
    let otpVerify = await pgConnect.executeQuery('loyalty',query);
